...
bool MSIMUreadGyroAxes(tSensors link, int &_x, int &_y, int &_z){
  ...
  if (!writeI2C(link, MSIMU_I2CRequest, MSIMU_I2CReply, 6))
    ...
    // wrong (and has been since I told him about it day 1, just goes to show true usage)
	  ...
	  _x = MSIMU_I2CReply[0] + ((MSIMU_I2CReply[1] >= 128) ? (int)MSIMU_I2CReply[1] - 256 : (int)MSIMU_I2CReply[1]) * 256;
	  _y = MSIMU_I2CReply[2] + ((MSIMU_I2CReply[3] >= 128) ? (int)MSIMU_I2CReply[3] - 256 : (int)MSIMU_I2CReply[3]) * 256;
	  _z = MSIMU_I2CReply[4] + ((MSIMU_I2CReply[5] >= 128) ? (int)MSIMU_I2CReply[5] - 256 : (int)MSIMU_I2CReply[5]) * 256;
